...
class ResetMetricsTest {
  ...
  void recordsOutcomesOverlapsRepeatsAndIsolatesWorlds() throws Exception {
    ...
    ResetMetrics metrics =
        new ResetMetrics(registry, List.of("resource", "nether"), "1.0.0", clock);
    ...
    metrics.onCompletion(
        post("two", "resource", ResetPhase.FAILED, ResetFailureType.EVENT_CANCELLED));
    metrics.onStart(pre("three", "nether"));
    clock.advanceSeconds(2);
    metrics.onCompletion(
        post("three", "nether", ResetPhase.FAILED, ResetFailureType.VERIFICATION_FAILED));
    metrics.onWarning(
        new ResourceWorldResetWarningEvent(
            "nether", "world_nether", 5, Instant.ofEpochSecond(500)));

    String text = exposition(registry);
    assertThat(text)
        .contains("rwr_reset_attempts_total{world=\"resource\",} 2.0")
  ...
  private static ResourceWorldPostResetEvent post(
    ...
    return new ResourceWorldPostResetEvent(
        operation,
        world,
        "world_" + world,
        phase,
        failure,
        failure == null ? FailureSafety.SAFE_TO_RETRY : FailureSafety.NOT_RETRYABLE,
        "done");
  }
